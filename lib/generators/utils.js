...
export function compileTemplate(content, variables, config) {
  ...
  let defaultTabSpace = 2;
  let tab = ' '.repeat(config.tabSpace || defaultTabSpace);

  // replace spaces followed by newline
  return compiled.replace(/\n( +)/g, function (match, p1) {
    let numTabs = p1.length/defaultTabSpace;
    return '\n' + tab.repeat(numTabs);
  });
}
...
export function _generate(type, moduleName, entityName, options) {
  ...
  let mantraConfig = readConfig();
  let component = compileTemplate(templateContent, templateVariables, mantraConfig);
